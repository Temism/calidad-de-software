		...
		if (empty($_POST['firstname2'])){
          ...
          )
         {
			...
			require_once ("../config/conexion.php");//Contiene funcion que conecta a la base de datos
				...
				$email_check_query = mysqli_query($con, "SELECT * FROM users WHERE user_email='$user_email' AND user_id != '$user_id'");
				if (mysqli_num_rows($email_check_query) > 0) {
					$errors[] = "El email ya está registrado por otro usuario.";
				} else {
					
					$username_check_query = mysqli_query($con, "SELECT * FROM users WHERE user_name='$user_name' AND user_id != '$user_id'");
